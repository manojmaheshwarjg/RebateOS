/**
 * Simple Direct Product Extraction Test
 * 
 * Uses Groq SDK directly without complex imports
 */
...
async function testSimpleExtraction(pdfPath: string) {
  console.log(`\n╔═══════════════════════════════════════════════════════════╗`);
...
**NOW EXTRACT ALL PRODUCTS AND RETURN JSON:**`;
  ...
  try {
    ...
    if (result.products && result.products.length > 0) {
      ...
      result.products.forEach((p: any, idx: number) => {
...
if (args.length === 0) {
  console.log(`\nUsage: npx tsx scripts/test-simple-extraction.ts <path-to-contract.pdf>`);
  console.log(`\nExample:`);
  console.log(`  npx tsx scripts/test-simple-extraction.ts contract_samples/Contract_1.pdf\n`);
  process.exit(1);
}
